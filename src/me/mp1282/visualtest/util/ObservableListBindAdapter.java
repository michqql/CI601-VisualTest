...
/**
 * Utility class to bind two {@code javafx.collections.ObservableList}'s together
 * while adapting the type of the source and target list
 * @param <S> The source type
 * @param <T> The target type
 */
public class ObservableListBindAdapter<S, T> {

    private final ObservableList<S> source;
    private final ObservableList<T> target;
    private final Function<S, T> factory;
    private final BiPredicate<S, T> removalPredicate;

    public ObservableListBindAdapter(ObservableList<S> source, ObservableList<T> target,
